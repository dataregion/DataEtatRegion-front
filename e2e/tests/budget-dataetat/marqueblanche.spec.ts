import { expect, Page, test, Request } from '@playwright/test';
import mockRefApi from '../utils/mock-api';

test.describe("Lorsque l'on définit le paramètre `grouper_par`", () => {

  const urlparam = '?programmes=101&annee_min=2023&annee_max=2023&grouper_par=theme,beneficiaire';
  ...
  test('Les colonnes de grouping sont pré-remplies', async ({ page }) => {
    ...
    const group_choices = page.getByTestId('group-modal_selected');
...
test.describe("Lorsque l'on définit le paramètre `grouper_par` invalide", () => {
  ...
  test("Une erreur s'affiche avec un message compréhensible", async ({ page }) => {
    ...
    const error_message = page.getByTestId('application-error-message');
...
test.describe("Lorsque l'on spécifie le plein écran", () => {
  const urlparam = `?programmes=101&annee_min=2023&annee_max=2023&grouper_par=theme,beneficiaire&plein_ecran=true`;
...
test.describe("Lorsque l'on spécifie des domaines fonctionnels", () => {
  ...
  test("Un message notifie que l'on recherche également sur le domaine fonctionnel", async ({
    page
  }) => {
    await _navigate(page, `/${urlparam}`);

    await expect(
      page.getByTestId('notif-additionnal-search-on-domaines-fonctionnels')
...
test.describe("Lorsque l'on spécifie des regions source", () => {
  ...
  test("Un message notifie que l'on recherche également sur les source de region", async ({
    page
  }) => {
    await page.goto(urlparam);

    await expect(page.getByTestId('notif-additionnal-search-on-source-region')).toContainText(
      'filtre sur la source region'
    );
  });
});

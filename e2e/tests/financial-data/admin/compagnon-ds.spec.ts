...
test.describe("Compagnon DS", () => {
    ...
    test("Vérification d'une démarche déjà intégrée", async ({ page }) => {
        ...
        await expect(page.getByTestId('dateIntegration')).toHaveText("Cette démarche a été intégrée le 29/07/2024");
    })
    
    test("Navigate by click => Réconciliation de la démarche + submit réconciliation EJ", async ({ page }) => {
        // Goto page d'intégration
        await page.goto('/administration/demarches/integration');

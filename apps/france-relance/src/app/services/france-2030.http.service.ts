...
export class France2030HttpService extends AbstractLaureatsHttpService {
  ...
  override getSousAxePlanRelance(): Observable<SousAxePlanRelanceForFilter[]> {
    ...
    return this.http.get<SousAxePlanRelance[]>(`${this.apiLaureats}/france-2030-axes`).pipe(
      map((axes) => {
        return axes.map((axe) => {
          const annotated = {
            ...axe,
  ...
  override searchStructure(structure: string): Observable<Structure[]> {
    ...
    return this.http.get<Structure[]>(`${this.apiLaureats}/france-2030-structures?term=${term}`);
  }
  ...
  override searchTerritoire(territoire: string): Observable<Territoire[]> {
    ...
    return this.http
      .get<
        Territoire[]
      >(`${this.apiLaureats}/france-2030-territoires?term=${term}`, { context: new HttpContext().set(DO_NOT_ALERT_ON_NON_IMPLEMTENTED, true) })
      .pipe(catchError(this._returnValueOn501([])));
  }

  override searchLaureats({
    axes,
    structure,
    niveau,
    territoires
    ...
    if (params_structures) url += `&structures=${encodeURIComponent(params_structures)}`;

    if (params_axes) url += `&axes=${encodeURIComponent(params_axes)}`;

    if (params_niveau) url += `&niveau_geo=${encodeURIComponent(params_niveau)}`;
